...
export default function ProyectosPorMes() {
  ...
  // MEDIDA DE SEGURIDAD: Se ha deshabilitado la eliminación de proyectos completos
  const handleBorrarProyecto = () => {
    alert("⚠️ La eliminación de proyectos está deshabilitada por seguridad.");
  };

  // MEDIDA DE SEGURIDAD: Se ha eliminado handleEliminarMes para evitar pérdida de datos históricos
                ...
                <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto p-1">
                  {editando.meses.map(m => (
                    <div key={m} className={`flex items-center gap-2 px-3 py-1.5 rounded-xl ${m === '9999-12' ? 'bg-yellow-100 border border-yellow-200' : 'bg-gray-100'}`}>
                      ...
                      {/* BOTÓN DE ELIMINAR MES ELIMINADO POR SEGURIDAD */}
                    </div>
                  ))}
                </div>
