...
export default function ProyectosPorMes() {
  ...
  const handleBorrarProyecto = () => {
    alert("⚠️ La eliminación de proyectos está deshabilitada por seguridad.");
  };
  ...
  const handleGuardarCambios = async (e) => {
    ...
    try {
      const resp = await fetch(`https://registromono.monognomo.com/api.php?action=edit_full_project_details`, {
        ...
        body: JSON.stringify(editando) // Se envía el objeto completo con el nuevo nombre
      });
    ...
    <div className="space-y-6 max-w-6xl mx-auto p-3 md:p-6 font-sans mb-6 text-left">
          ...
          <div key={company.id} className="bg-white rounded-4xl shadow-sm border border-gray-50 overflow-hidden text-left">
            ...
            {abiertos[company.id] && (
              <div className="overflow-x-auto text-left">
                <table className="hidden md:table w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {company.projects.map((p) => (
                      <tr key={p.id} className="hover:bg-gray-50/30 transition-colors align-top text-left">
                        <td className="px-8 py-5 text-xs font-bold uppercase text-gray-800 leading-relaxed text-left">{p.name}</td>
                        ...
                        <td className="px-8 py-5 text-[10px] text-gray-400 italic leading-relaxed text-left">
                            <span className="not-italic mr-1 text-left">👥</span>
                            {p.display_workers}
                        </td>
                ...
                {/* VISTA MÓVIL */}
                <div className="md:hidden divide-y divide-gray-100 text-left">
                  {company.projects.map((p) => (
                    <div key={p.id} className="p-5 space-y-3 text-left">
                      <div className="flex justify-between items-start gap-4 text-left">
                        <span className="font-black text-gray-800 text-[11px] uppercase leading-tight text-left">{p.name}</span>
                      ...
                      <p className="text-[9px] text-gray-400 italic font-medium leading-normal text-left">👥 {p.display_workers}</p>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {modalAbierto && (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm text-left">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95 duration-200 text-left">
            <div className="flex justify-between items-center mb-8 text-left">
              <div className="text-left">
                <h3 className="text-[11px] font-black uppercase tracking-widest text-gray-800 text-left">Editor de Proyecto</h3>
                <p className="text-[9px] text-gray-400 font-bold uppercase mt-1 text-left line-clamp-1">{editando.name}</p>
              </div>
            ...
            <form onSubmit={handleGuardarCambios} className="space-y-6 text-left">
              ...
              <div className="text-left">
                <div className="flex justify-between items-center mb-2 text-left">
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1 text-left">Meses Activos</label>
                ...
                <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto p-1 text-left">
                  {editando.meses.map(m => (
                    <div key={m} className={`flex items-center gap-2 px-3 py-1.5 rounded-xl ${m === '9999-12' ? 'bg-yellow-100 border border-yellow-200' : 'bg-gray-100'} text-left`}>
                      <span className="text-[9px] font-bold text-gray-600 uppercase text-left">{formatearMesAnio(m)}</span>
                    </div>
                  ))}
                </div>
                <div className="mt-4 flex gap-2 text-left">
